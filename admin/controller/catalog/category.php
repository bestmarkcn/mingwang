...
class ControllerCatalogCategory extends Controller { 
	...
	protected function getList() {
		...
		$data = array(
		    'filter_level' =>isset($this->request->get['filter_level'])?$this->request->get['filter_level']:null,
	...
	protected function getForm() {
		...
		if (isset($this->request->post['icon']) && file_exists(DIR_IMAGE . $this->request->post['icon'])) {
			$this->data['icon_thumb'] = $this->model_tool_image->resize($this->request->post['icon'], 100, 100);
		} elseif (!empty($category_info) && $category_info['icon'] && file_exists(DIR_IMAGE . $category_info['icon'])) {
			$this->data['icon_thumb'] = $this->model_tool_image->resize($category_info['icon'], 100, 100);
		} else {
		...
		if (isset($this->request->post['file_jpg'])) {
			$this->data['file_jpg'] = $this->request->post['file_jpg'];
		} elseif (!empty($category_info)) {
		...
		if (isset($this->request->post['file_ai'])) {
			$this->data['file_ai'] = $this->request->post['file_ai'];
		} elseif (!empty($category_info)) {
		...
		if (!isset($json['error'])) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				...
				$json['filename'] = $new_filename;
 				$json['mask'] = $filename;
 				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_DOWNLOAD . $new_filename );
			}
