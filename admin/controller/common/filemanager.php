...
class ControllerCommonFileManager extends Controller {
	...
	public function index() {
		...
		$this->data['directory'] =  'image/data/'; //改成相對路徑  UPDATE  BY  HONG
	...
	public function fck_upload() {
		...
		$json = array();
		if (!isset($this->request->post['directory'])) {
            $this->request->post['directory']='fck';
        }
 		if (isset($this->request->post['directory'])) {
			if (isset($this->request->files['upload']) && $this->request->files['upload']['tmp_name']) {
				...
				if ($this->request->files['upload']['size'] > 3000000) {
					$json['error'] = $this->language->get('error_file_size');
				}
		...
		if (!isset($json['error'])) {	
		    $callback= "<script>window.parent.CKEDITOR.tools.callFunction(4, 'image/data/".$this->request->post['directory']. '/' . $filename."' );</script>";

		}else{
		    $callback= "<script>alert('".$json['error']."');</script>";
		} 
		 
 		 $this->response->setOutput($callback);
	}
