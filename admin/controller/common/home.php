	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/inquiry/update', 'token=' . $this->session->data['token'] . '&inquiry_id=' . $result['inquiry_id'], 'SSL')
			);

			$this->data['orders'][] = array(
				'order_id'   => $result['inquiry_id'],
				'customer'   => $result['name'],
				'status'     => $result['status']?'已處理':'未處理',
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/contact/update', 'token=' . $this->session->data['token'] . '&contact_id=' . $result['contact_id'] , 'SSL')
			);

			$this->data['contacts'][] = array(
 				'order_id'   => $result['contact_id'],
				'customer'   => $result['name'],
				'status'     => $result['status']?'已處理':'未處理',
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
 				'action'     => $action
