...
class ControllerSaleCustomer extends Controller { 
	...
	public function importCustomer() {
	  	   ...
	  	   $this->load->helper('csvread');
           $upsavepath=DIR_IMAGE.'customercsv/';
           $filename="shennao1.csv";
 		   $this->load->model('localisation/country');
 		   $this->load->model('sale/customer');
 		   $this->load->model('localisation/zone');
		   $csv = new HelperCsvread($upsavepath. $filename);
		   
		   $csv->open(0);
 		   $csv_list = $csv->getRecords();
		   $csv->close();

            foreach ($csv_list as $val){
                   foreach($val as $key=>$value){
	  			       if(!$this->isUTF8($value)){
	 			             $val[$key]=mb_convert_encoding($value,'utf-8','gb2312');
	  				   }
			       }
		           $country = $this->model_localisation_country->getCountryByName(trim($val[6]));
				   $country_id=$country['country_id'];
 		           $zone = $this->model_localisation_zone->getZoneByName(trim($val[7]));
				   $zone_id=$zone['zone_id'];
  				   $data=array(
 				           'customer_group_id'=>'3',
 				           'status'=>'1',
 				           'approved'=>'1',
 				           'email'=>addslashes($val[0]),
  				           'password'=>addslashes($val[1]),
 				           'firstname'=>addslashes($val[3]),
 				           'company'=>addslashes($val[3]),
 				           'shop'=>addslashes($val[0]),
 				           'telephone'=>addslashes($val[9]),
 				           'address_1'=>addslashes($val[8]),
     				       'country_id'=>$country_id,
     				       'zone_id'=> $zone_id
   				    );
					
				 
			    $this->model_sale_customer->addImportCustomer($data);
   ...
   public function output() {
		...
		$data='';
		$filename = time().'.csv';
 			$data.= $this->iconvstr('客戶名稱').",".$this->iconvstr('帳號').",".$this->iconvstr('用戶組').",".$this->iconvstr('電話').",".$this->iconvstr('傳真').",".$this->iconvstr('地址')."\r\n";
		...
		foreach($results as $customer){		
					 ...
					 $default_address=$address['country'].$address['zone'].$address['address_1'];
					
					$data.= $this->iconvstr($customer['firstname']).",".$this->iconvstr($customer['email']).",".$this->iconvstr($customer['customer_group']).",".$this->iconvstr($customer['telephone']).",".$this->iconvstr($customer['fax']).",".$this->iconvstr( $default_address) ;
 ...
 //exit;
	...
	function escapeCSV($str){
 	...
 	function iconvstr($str){
		//return $str;
	   return iconv('UTF-8','gbk',$this->escapeCSV($str));
 	}
