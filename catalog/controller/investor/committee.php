<?php 
class ControllerInvestorCommittee extends Controller {  
	public function index() { 
			$this->document->setTitle("功能性委員會");
			$this->load->model('investor/investor');
			$filter_ask_type_id=8;
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/investor/committee.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/investor/committee.tpl';
			} else {
				$this->template = 'default/template/investor/committee.tpl';
			}
