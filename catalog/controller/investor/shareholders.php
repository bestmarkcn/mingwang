<?php 
class ControllerInvestorShareholders extends Controller {  
	public function index() { 
			$this->document->setTitle("股東會資料");
			$this->load->model('investor/investor');
			$filter_ask_type_id=11;
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/investor/shareholders.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/investor/shareholders.tpl';
			} else {
				$this->template = 'default/template/investor/shareholders.tpl';
			}
