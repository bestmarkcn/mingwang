...
//  		$this->data['action'] = 'https://test.esafe.com.tw/Service/Etopm.aspx';
		...
		$this->data['note2'] = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_24payment/success';
	...
	public function success() {
		...
		if (isset($data['note2'])) {
            ...
            if ($isSuccess) {
                ...
                //檢查送出之hash與金額
                $total_amount = intval($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false));
			    $output = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_24payment/success';
			...
			if ($isSuccess) {
				if ($data['errcode'] == '00') {
				    ...
				    $message = sprintf($this->language->get('text_failure_notify').'', $data['buysafeno'], $data['errcode']);
				    if (!$order_info['order_status_id']) {
			        ...
			        $this->data['text_message'] = sprintf($this->language->get('text_failure'),$data['errcode']);
			        $this->data['text_message_wait'] = sprintf($this->language->get('text_failure_wait'), $this->url->link('checkout/checkout', '', 'SSL'));
				}
			}
		}
