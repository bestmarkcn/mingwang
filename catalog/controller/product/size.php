		...
		if ($category_info) {
			...
			if (isset($this->request->get['limit'])) {
				...
				foreach ($product_results as $product_result) {
 			        $attributes = $this->model_catalog_product->getProductAttributes($product_result['product_id']);
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/size.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/product/size.tpl';
			} else {
