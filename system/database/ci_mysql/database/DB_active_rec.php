...
class CI_DB_active_record extends CI_DB_driver {
	...
	function set($key, $value = '', $escape = TRUE)
	{
	 ...
	 /************
	   過濾掉多餘的字段 ADD by hong
	 *************************/
	function filter_fields($key, $table)
	{
 		foreach ($key as $k => $v)
		{
  			if(!$this->field_exists($k,$table)){
 				unset($this->ar_set[$this->_protect_identifiers($k)]);
 			}
		}
  		return $this;
	}
	...
	function update($table = '', $set = NULL, $where = NULL, $limit = NULL)
	{
		...
		if ($limit != NULL)
		{
		 ...
		 $this->filter_fields($set,$table); //過濾掉多餘的字段
